    ...
    function () {
        ...
        if (has('previous_release')) {
            run(
                ...
                '{{deploy_path}}/releases/' . $releases[1] . '/{{public_folder}}/{{opcache_reset_url}}.php'
            );
            run(
                'echo "<?php opcache_reset(); ?>" > ' .
                '{{release_path}}/{{public_folder}}/{{opcache_reset_url}}.php'
            );
...
task(
    ...
    function () {
        ...
        if (has('previous_release')) {
            run(
                'echo "<?php clearstatcache(); ?>" > ' .
                '{{deploy_path}}/releases/' . $releases[1] . '/{{public_folder}}/{{clearstatcache_url}}.php'
            );
            run(
                'echo "<?php clearstatcache(); ?>" > ' .
                '{{release_path}}/{{public_folder}}/{{clearstatcache_url}}.php'
            );
